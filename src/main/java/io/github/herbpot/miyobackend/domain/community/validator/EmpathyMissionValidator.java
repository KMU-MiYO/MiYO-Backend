...
public class EmpathyMissionValidator implements MissionValidator {
    ...
    public void validateAndUpdateProgress(String userId, Long relatedId) {
        log.info("Validating empathy mission: userId={}, empathyId={}", userId, relatedId);

        LocalDate today = LocalDate.now();
        List<Mission> activeMissions = missionRepository.findActiveMissionsByCategory("empathy", today);

        if (activeMissions.isEmpty()) {
            log.debug("No active empathy missions found");
            return;
        }
        ...
        for (Mission mission : activeMissions) {
            ...
            log.info("Empathy mission progress updated: userId={}, missionId={}, currentCount={}, completed={}",

...
public class PostReadService {
    ...
    public Page<PostListResponse> findPostsByLocation(
        ...
        java.util.List<PostListResponse> responseList = sortedList.stream()
                .map(model -> {
                    String nickname = userRepository.findNicknameById(model.getUserId());
    ...
    public PostDetailResponse findPostById(Long postId, Long userId) {
        ...
        // 닉네임 조회
        String nickname = userRepository.findNicknameById(readModel.getUserId());
        if (nickname == null) {
            nickname = "Unknown";
        }
     ...
     * - 1단계: 주어진 좌표 기준 반경 내 게시글 조회 (ST_Distance_Sphere)
     * - 2단계: 조회된 게시글들을 지역 경계(GeoJSON Polygon) 필터링
     * - regionName이 null이면 반경 검색만 수행
     *
     * @param latitude 검색 중심 위도
     * @param longitude 검색 중심 경도
     * @param radiusKm 검색 반경 (km, nullable)
    ...
    public Page<PostListResponse> findPostsByRegion(
        ...
        log.info("Finding posts by region: lat={}, lng={}, radius={}km, region={}, page={}",
                latitude, longitude, radius, regionName, pageable.getPageNumber());
    ...
    /**
     * 지역별 TOP 3 게시글 조회
     * - 주어진 좌표 기준으로 반경 내 게시글 조회
     * - 공감수 기준으로 내림차순 정렬하여 상위 3개 반환
     * - 동네 정보는 regionName으로 표시
     *
     * @param latitude 검색 중심 위도
     * @param longitude 검색 중심 경도
     * @param radiusKm 검색 반경 (km, nullable)
     * @param regionName 행정구역명 (예: "서울시 강남구 역삼동")
    ...
    public java.util.List<PostListResponse> findTop3PostsByLocation(
        ...
        log.info("Finding top 3 posts by location: lat={}, lng={}, radius={}km, region={}",
                latitude, longitude, radius, regionName);
        ...
        java.util.List<PostReadModel> top3List = readModels.getContent().stream()
                .sorted((a, b) -> {
                    Long countA = empathyCountMap.getOrDefault(a.getPostId(), 0L);
                    Long countB = empathyCountMap.getOrDefault(b.getPostId(), 0L);
                    int countCompare = countB.compareTo(countA); // 내림차순
